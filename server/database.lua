...
function Database.loadFromFile()
    ...
    if file then
        ...
        if content and content ~= '' then
            ...
            if success then
                Database.detections = result or {}
                Utils.logSuccess('Loaded ' .. #Database.detections .. ' detection records from file')
            else
                Utils.logError('Failed to decode detection file: ' .. tostring(result))
                Database.detections = {}
            end
        end
    else
...
function Database.saveToFile()
    ...
    local dir = Config.Database.filePath
    ...
    if file then
        local success, encoded = pcall(json.encode, Database.detections)
        if success then
            file:write(encoded)
            file:close()
            return true
        else
            Utils.logError('Failed to encode detection data: ' .. tostring(encoded))
            file:close()
            return false
        end
    else
        Utils.logError('Failed to save detection history to file: ' .. path)
        return false
    end
end
